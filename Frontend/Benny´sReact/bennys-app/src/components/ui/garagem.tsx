...
const GaragePage = () => {
    ...
    async function onSubmit(data: any) {
        ...
        const idVeiculo = selectedCar?.id;

        if (idVeiculo) {
            formData.idVeiculo = idVeiculo;

            try {
                const response = await fetch('http://localhost:8080/veiculo/disponibilidade', {
                    method: 'POST',
                    headers: {
                ...
                const respostaArray = await response.json();
                const resposta = respostaArray[0];

                if (resposta.status === "erro") {
                    toast({
                        variant: "destructive",
                        title: "Erro ao colocar o veículo à venda.",
                        description: resposta.mensagem || "Ocorreu um erro ao colocar o veículo à venda."
                    });
                } else if (resposta.status === "sucesso") {
                    toast({
                        variant: "success",
                        title: "Veículo colocado à venda com sucesso.",
                        description: resposta.mensagem || "Veículo colocado à venda com sucesso."
                    });
                    setSubmitted(true);
                }
            } catch (error: any) {
                toast({
                    variant: "destructive",
                    title: "Erro",
                    description: error.message || "Ocorreu um erro ao comunicar com o servidor. Por favor, tente novamente."
                });
                            ...
                            <Form {...form}>
                                <div className="grid gap-4 py-4">
                                    ...
                                    <div className="flex justify-end">
                                        <Button size="sm" type="submit" onClick={form.handleSubmit(onSubmit)}>Vender</Button>
                                    </div>
                                </div>
                            </Form>
                        </DialogContent>
                ...
                return (
                    <>
                        <DialogContent className="rounded max-w-5xl">
                            <DialogHeader>
                                <DialogTitle>Localização do Carro</DialogTitle>
                                <DialogDescription>Informações sobre a localização do carro.</DialogDescription>
                            </DialogHeader>
                            <div className="grid gap-4 py-4">
                                {/* Conteúdo específico para a localização */}
                                <p>Informações sobre a localização do carro {selectedCar.Fabricante} {selectedCar.modelo}</p>
                            </div>
                        </DialogContent>
                    </>
                );
            case 'seguro':
                return (
                    <>
                        <DialogContent className="rounded max-w-5xl">
                            <DialogHeader>
                                <DialogTitle>Seguro do Carro</DialogTitle>
                                <DialogDescription>Detalhes sobre o seguro do carro.</DialogDescription>
                            </DialogHeader>
                            <div className="grid gap-4 py-4">
                                {/* Conteúdo específico para o seguro */}
                                <p>Informações sobre o seguro do carro {selectedCar.Fabricante} {selectedCar.modelo}</p>
                            </div>
                        </DialogContent>
                    </>
                );
            case 'acidentes':
                return (
                    ...
                    <>
                        <DialogContent className="rounded max-w-5xl">
                            <DialogHeader>
                                <DialogTitle>Histórico de Acidentes</DialogTitle>
                                <DialogDescription>Informações sobre o histórico de acidentes do carro.</DialogDescription>
                            </DialogHeader>
                            <div className="grid gap-4 py-4">
                                {/* Conteúdo específico para os acidentes */}
                                <p>Histórico de acidentes do carro {selectedCar.Fabricante} {selectedCar.modelo}</p>
                            </div>
                        </DialogContent>
                    </>
                );
                        ...
                        {carData.map((car) => (
                            <DropdownMenu key={car.id}>
                                    ...
                                    <Card key={car.id} className="card cursor-pointer">
                                        <CardHeader>
                                            <CardTitle className="flex justify-between items-center">
                                                ...
                                                {car.disponibilidade === "Disponível" && (
                                ...
                                <DropdownMenuContent align="center" side="right">
                                    ...
                                    <Separator />
                                    <DropdownMenuItem onClick={() => openDialog('vender', car)}>Vender</DropdownMenuItem>
                                    <DropdownMenuItem onClick={() => openDialog('localizacao', car)}>Localização</DropdownMenuItem>
                                    <DropdownMenuItem onClick={() => openDialog('seguro', car)}>Seguro</DropdownMenuItem>
                                    <DropdownMenuItem onClick={() => openDialog('acidentes', car)}>Acidentes</DropdownMenuItem>
